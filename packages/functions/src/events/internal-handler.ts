...
import { ProjectRegistryKey, PROJECT_REGISTRY, payoutMetadataSchema } from "@normietech/core/config/project-registry/index";
...
import { transactions, payoutSettings, projects, payoutBalance } from "@normietech/core/database/schema/index";
...
const handleOnChainTransaction = async (metadata: z.infer<typeof metadataSquareSchema>,payment:Payment) => {
    ...
    let finalFiatAmountInCents =
      Number.parseInt(payment.amountMoney?.amount?.toString() ?? "0");
